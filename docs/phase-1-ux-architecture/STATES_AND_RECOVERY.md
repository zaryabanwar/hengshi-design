...
| ACT-09 Select delivery stream | Chosen stream applied only after explicit Apply activation by Enter, Space, pointer or touch, or native Enter submission of the same form to the Apply button; group accessible name reports the new in-force stream; location preserved; native radio anatomy and advance advisement follow `FR-3D-014` and `NFR-A11Y-004` (D-045; style guide §8.2.4) | Before Apply, checked radio reports the pending selection while the group accessible name still reports the in-force stream. Arrow, Tab, pointer and touch selection change only checked state: no stream application, reload, transition or other accessibility-tree change; never apply on focus, selection, blur, arrow movement or timeout. Failure states: requested stream above the WebGL ceiling; change failure; preference cannot persist | remain on the prior stream and say why; the ceiling is stated, not silently substituted; persistence failure follows ACT-11 and the choice still applies for the session; no content loss in any stream |
