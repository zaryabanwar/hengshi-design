...
| CN-012 | World/3D | project-owned source/provenance, canonical mappings, accessible labels, quality tiers, loader/fallback content | Existing GLB reference-only; ECL-022; MA-004 | Semantic routes and reserved room mapping; no geometry reuse |
...
| HYP-02 | Optional World improves orientation/memorability for some visitors without reducing semantic completion. | Cohort-sized usability findings and KPI-D06-style aggregate outcomes | No parity threshold/causality claim; never route users to a worse path. |
