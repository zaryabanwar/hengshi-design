...
const profileRoot = path.join(os.tmpdir(), `hengshi-brand-d034-contrast-completion-${process.pid}-${Date.now()}`);
const textSpacingCss = '* { line-height:1.5 !important; letter-spacing:.12em !important; word-spacing:.16em !important; } p { margin-bottom:2em !important; }';
...
async function capture(name, url, viewport, options = {}) {
  ...
  try {
    ...
    if (options.assetFailure) await page.route('**/*.svg', route => route.abort('failed'));
    ...
    if (options.textSpacing) await page.addStyleTag({ content: textSpacingCss });
...
const jobs = [
  ...
  ['identity-board-320.png', boardUrl, { width: 320, height: 800 }, {}],
  ['identity-board-text-spacing-320.png', boardUrl, { width: 320, height: 800 }, { textSpacing: true }],
  ['identity-board-asset-failure-320.png', boardUrl, { width: 320, height: 800 }, { assetFailure: true }],
