...
foreach($file in $files){
    ...
    $entries+=[ordered]@{
        ...
        origin=if($isCapture){'Local Playwright capture of project-authored iteration-3 identity evidence; no network request permitted'}else{'Original hand-authored project-local specimen implementing D-026, D-027, and CR-001'}
        format=$format
...
$manifest=[ordered]@{
    ...
    document=[ordered]@{
        ...
        version='producer-iteration-3-final'
        date='2026-07-19'
        status='internal_review_only'
        defaultCreator='Hengshi Phase 1 identity producer'
        defaultOrigin='Original hand-authored project-local specimen implementing D-026, D-027, and CR-001'
